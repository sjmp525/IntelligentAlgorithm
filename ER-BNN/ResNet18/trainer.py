...
parser.add_argument('--lr', '--learning_rate', default=0.10, type=float,
...
parser.add_argument('--resume', default='none', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')# D:\BNN\IM-BNN\IM-BNN\ResNet18\saveModel-002\Best-checkpoint.th
...
parser.add_argument('--print_freq', '-p', type=int, default=150,
                    metavar='N', help='print frequency (default: 200)')
parser.add_argument('--save_every', dest='save_every', type=int, default=200,
                    help='Saves checkpoints at every specified number of epochs (default: 200)')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Tensorboard 可视化 权重分布
        if (epoch + 1) % 100 == 0:
            for name, param in model.named_parameters():
                SumWriter.add_histogram(name, param.data.cpu().numpy(), epoch)
...
def save_checkpoint(epoch, save_every, state, is_best, save):
    ...
    """
    filename = os.path.join(save, str(epoch) + '-checkpoint.th')
