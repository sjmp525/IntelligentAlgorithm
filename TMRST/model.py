...
class MECTNER(nn.Module):
    ...
    def forward(self, lattice, bigrams, seq_len, lex_num, pos_s, pos_e, target):
        ...
        # m = fusion.cpu().detach().numpy()
        # map = m[0, ...]
        # ax = sns.heatmap(np.multiply(map, 40), vmin=0, vmax=1, cmap="RdBu")  # 原来的是20或者30
        # # # ax = sns.heatmap(A_C[:,:,0,0].cpu().detach().numpy(), fmt="d", cmap='YlGnBu', vmin=0, vmax=1)
        # plt.show()
        # fusion = torch.tensor(fusion).cuda()
