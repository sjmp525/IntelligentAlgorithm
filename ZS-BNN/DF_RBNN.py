...
def train(args, teacher, student, generator, device, optimizer, epoch):
    ...
    for i in range(args.epoch_itrs):
        ...
        # if (epoch + 1) % 100 == 0:
        # image = vutils.make_grid(fake[12].float(), nrow=16 , normalize=True)
        # SumWriter.add_image("image", image, epoch)
        #
        # f_feat_s = vutils.make_grid(feat_s[0][12].float().detach().cpu().unsqueeze(dim=1), nrow=8, normalize=True)
        # SumWriter.add_image("f_feat_s", f_feat_s, epoch)
        #
        # L1_feat_s = vutils.make_grid(feat_s[1][12].float().detach().cpu().unsqueeze(dim=1), nrow=8, normalize=True)
        # SumWriter.add_image("L1_feat_s", L1_feat_s, epoch)
        #
        # L2_feat_s = vutils.make_grid(feat_s[2][12].float().detach().cpu().unsqueeze(dim=1), nrow=11, normalize=True)
        # SumWriter.add_image("L2_feat_s", L2_feat_s, epoch)
        #
        # L3_feat_s = vutils.make_grid(feat_s[3][12].float().detach().cpu().unsqueeze(dim=1), nrow=16, normalize=True)
        # SumWriter.add_image("L3_feat_s", L3_feat_s, epoch)
        #
        # L4_feat_s = vutils.make_grid(feat_s[4][12].float().detach().cpu().unsqueeze(dim=1), nrow=22, normalize=True)
...
def main():
    ...
    parser.add_argument('--Tmin', default=1e-2, type=float, metavar='M', help='minimum of T (default:1e-2)')
    parser.add_argument('--Tmax', default=1e1, type=float, metavar='M', help='maximum of T (default:1e1)')
    ...
    # student.load_state_dict(torch.load('/home/ghw/DF_ABNN/RBNN/rerun_l1loss/checkpoint/student/resnet20_RBNN_2step_cifar10.pt'))
    # print("buffer_a_2step.pt")

    # generator.load_state_dict((torch.load('/home/ghw/DF_ABNN/RBNN/rerun_l1loss/checkpoint/student/generator_resnet20_RBNN_2step_cifar10.pt')))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(student.state_dict(), "RBNN/checkpoint/student/test.pt")
            torch.save(generator.state_dict(), "RBNN/checkpoint/student/generator_test.pt")
        # vp.add_scalar('Acc', epoch, acc)
    print("Best Acc=%.6f" % best_acc)

    # import csv
    # os.makedirs('log', exist_ok=True)
    # with open('log/DFAD-%s_resnet20_RBNN_1step.csv' % (args.dataset), 'a') as f:
    #     writer = csv.writer(f)
