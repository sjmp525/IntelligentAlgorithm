    ...
    def __init__(self, inp, oup, stride, dilation, expand_ratio, BatchNorm):
        ...
        if expand_ratio == 1:
            self.conv = nn.Sequential(
                ...
                DA(hidden_dim, hidden_dim, 3, stride, 0, dilation, groups=hidden_dim, bias=False),
                BatchNorm(hidden_dim),
                nn.ReLU6(inplace=True),
                # pw-linear
                # nn.Conv2d(hidden_dim, oup, 1, 1, 0, 1, 1, bias=False),
                DA(hidden_dim, oup, 1, 1, 0, 1, 1, bias=False),
                BatchNorm(oup),
            )
        else:
            self.conv = nn.Sequential(
                ...
                DA(inp, hidden_dim, 1, 1, 0, 1, bias=False),
                BatchNorm(hidden_dim),
                nn.ReLU6(inplace=True),
                # dw
                # nn.Conv2d(hidden_dim, hidden_dim, 3, stride, 0, dilation, groups=hidden_dim, bias=False),
                ...
                DA(hidden_dim, oup, 1, 1, 0, 1, bias=False),
                BatchNorm(oup),
            )
